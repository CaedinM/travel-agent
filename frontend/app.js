...
function App() {
  ...
  const resetConversation = () => { setMessages([]); setThreadId(null); setError(""); };
  return <main className="app-shell"><header className="topbar"><a className="wordmark" href="/" aria-label="Travel Agentic home">Travel Agentic<span>.</span></a><button className="new-chat" onClick={resetConversation}>New conversation</button></header><MessageStream messages={messages} isSending={isSending} error={error} onStarter={sendMessage} /><ChatComposer disabled={isSending} onSend={sendMessage} /></main>;
}
